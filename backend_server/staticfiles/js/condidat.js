function searchStage() {
    ...
    for (i = 0; i < tr.length; i++) {
        ...
        td2 = tr[i].getElementsByTagName("td")[4]; // Get the second column (created_at)
        if (td1 && td2) {
...
function filterTypeStage(transport) {
    var rows = document.querySelectorAll('.condidat-row');
    
    rows.forEach(function(row) {
        var rowtransport = row.getAttribute('data-transport');
        
        if (transport === '' || rowtransport === transport) {
