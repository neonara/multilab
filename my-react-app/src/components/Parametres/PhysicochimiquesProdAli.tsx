 ...
 /*  useEffect(() => {
  ...
  const fetchParameters = async () => {
    ...
    try {
      ...
      setCurrentData(extractedParameters.filter(Boolean));
    } catch (err) {
  ...
  const renderAnalyseList = (analysesList: typeof analyses) => (
    <div className="analyses-container">
      {analysesList.map((analyse) => {
        ...
        const shouldShowElements = isCurrentAnalysis && openIndex === analyse.key;

        console.log('shouldShowElements' , shouldShowElements, 'id' , id ,'analyse.key', analyse.key  , 'openIndex', openIndex , 'analyse.id;' ,analyse.id);
        return (
          <div
            key={analyse.id}
            className={`analyse-item ${openIndex === analyse.key ? "analyse-item-on" : ""}`}
          >
            <div className="analyse-title" onClick={() => toggleFAQ(analyse.key)}>
              ...
              <div className="expand-btn">
                ...
                <FiMinus
                  className={`expand-icon-rotate ${openIndex === analyse.key ? "expand-icon-on" : ""}`}
                />
              </div>
            </div>
            <div
              className={`analyse-details ${openIndex === analyse.key ? "analyse-details-on" : ""}`}
            >
              {shouldShowElements ? (
                isLoading ? (
                  <p>Loading...</p>
