...
import {TypeAnalysesAlimentationAnimale} from "../../types/types";
...
const chunkArray = (array: TypeAnalysesAlimentationAnimale[], size: number) => {
  const chunked = [];
...
const Card = ({ items }: { items: TypeAnalysesAlimentationAnimale[] }) => (
  <div className="card">
   <ul className="space-y-2">
        {items.map((item, index) => (
          ...
          >
            {item.type_alimentation_animale}
          </li>
        ))}
      </ul>
  </div>
...
const AnalysesAlimentationAnimale = () => {
  const [analyses, setAnalyses] = useState<TypeAnalysesAlimentationAnimale[]>([]);
  const getAnalyses = async() => {
   
    try {
      const response = await api.get('devi');
...
export default AnalysesAlimentationAnimale;
