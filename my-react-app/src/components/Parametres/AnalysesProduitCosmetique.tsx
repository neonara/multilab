...
import {TypeAnalysesProduitCosmetique} from "../../types/types";
...
const chunkArray = (array: TypeAnalysesProduitCosmetique[], size: number) => {
  const chunked = [];
...
const Card = ({ items }: { items: TypeAnalysesProduitCosmetique[] }) => (
  <div className="card">
   <ul className="space-y-2">
        {items.map((item, index) => (
          ...
          >
            {item.type_analyses_microbiologiques_produits_cosmetique}
          </li>
        ))}
      </ul>
  </div>
...
const AnalysesProduitCosmetique = () => {
  const [analyses, setAnalyses] = useState<TypeAnalysesProduitCosmetique[]>([]);
  const getAnalyses = async() => {
   
    try {
      const response = await api.get('/cosmetique');
  ...
  return (
    <div className="container">
      <h2>Analyses Microbiologiques des produits cosmétiques et D’hygiène</h2>
...
export default AnalysesProduitCosmetique;
