...
import {TypeAnalysesPhysicochimiquesProduitsEaux} from "../../types/types";
...
const chunkArray = (array: TypeAnalysesPhysicochimiquesProduitsEaux[], size: number) => {
  const chunked = [];
...
const Card = ({ items }: { items: TypeAnalysesPhysicochimiquesProduitsEaux[] }) => (
  <div className="card">
   <ul className="space-y-2">
        {items.map((item, index) => (
          ...
          >
            {item.type_analyses_physicochimiques_produits_eaux}
          </li>
        ))}
      </ul>
  </div>
...
const AnalysesPhysicochimiquesProduitsEaux = () => {
  const [analyses, setAnalyses] = useState<TypeAnalysesPhysicochimiquesProduitsEaux[]>([]);
  const getAnalyses = async() => {
   
    try {
      const response = await api.get('/physico-eaux');
  ...
  return (
    <div className="container">
      <h2>Paramètre d’analyses Physicochimiques des Produits eaux</h2>
...
export default AnalysesPhysicochimiquesProduitsEaux;
